    ...
    void canCreateAndVerifyRegisteredNodeWithPolling() throws Exception {
        try (var testEnv = new IntegrationTestEnv(1)) {
            ...
            List<RegisteredServiceEndpoint> serviceEndpoints = List.of(
                new BlockNodeServiceEndpoint().setDomainName("test.block.com").setPort(443).addEndpointApi(BlockNodeApi.STATUS),
                new MirrorNodeServiceEndpoint().setIpAddress(new byte[]{127, 0, 0, 1}).setPort(443),
                new RpcRelayServiceEndpoint().setDomainName("test.rpc.com").setPort(443),
                new GeneralServiceEndpoint().setDomainName("test.general.com").setPort(8080)
            );
