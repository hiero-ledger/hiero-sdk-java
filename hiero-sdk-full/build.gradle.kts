...
val sdkSrcMainProto = layout.projectDirectory.dir("../hiero-sdk/src/main/proto")
val sdkSrcMainJava =
    layout.projectDirectory.dir("../hiero-sdk/src/main/java").asFileTree.matching {
        exclude("module-info.java")
    }
val sdkSrcMainResources = layout.projectDirectory.dir("../hiero-sdk/src/main/resources")
...
// 'hiero-sdk-full' is an alternative to 'hiero-sdk'. They cannot be used together.
// We express this via capability.
listOf(configurations.apiElements.get(), configurations.runtimeElements.get()).forEach {
    // The 'hiero-sdk-full' capability (default)
    it.outgoing.capability("${project.group}:${project.name}:${project.version}")
    // The 'hiero-sdk' capability
    it.outgoing.capability("${project.group}:hiero-sdk:${project.version}")
}
